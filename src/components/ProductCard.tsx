  ...
  return (
    <div className="group bg-white border border-[#E7E5E4] luxury-card-shadow flex flex-col justify-between h-full relative transition-all duration-300">
        ...
        <div className="relative aspect-[3/4] bg-[#FAF8F5] overflow-hidden image-zoom-container">
          ...
          <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2">
            ...
            <button
              ...
              className="bg-[#7A1C30] hover:bg-[#5F1524] text-white text-[11px] uppercase tracking-wider px-3 py-2 font-bold flex items-center gap-1.5 transition-colors"
            >
          ...
          <div className="flex items-baseline space-x-2 pt-1">
            {product.price ? (
              <>
                {product.salePrice ? (
                  <>
                    <span className="font-bold text-sm text-[#7A1C30]">₹{product.salePrice.toLocaleString('en-IN')}</span>
                    <span className="text-xs text-[#78716C] line-through">₹{product.price.toLocaleString('en-IN')}</span>
                  </>
                ) : (
                  <span className="font-bold text-sm text-[#1C1917]">₹{product.price.toLocaleString('en-IN')}</span>
                )}
              </>
            ) : (
      ...
      <div className="px-4 pb-4 pt-1">
        <button
          onClick={handleAddToCart}
          className="w-full bg-[#7A1C30] hover:bg-[#5F1524] text-white transition-colors text-xs uppercase tracking-widest py-2 font-bold flex items-center justify-center gap-2"
        >
          <ShoppingBag size={14} /> Add to Cart
        </button>
      </div>
